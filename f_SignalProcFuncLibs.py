...
def f_GaborTFTransform(p_XIn, p_FsHz, p_F1Hz, p_F2Hz, p_FreqResHz, p_NumCycles):
    ...
    # En esta matriz cada fila corresponde al resultado de una convolución y cada columna a todos
    # los desplazamientos de tiempo.
    m_ConvMat = np.zeros([np.size(v_FreqTestHz), np.size(p_XIn)], dtype=complex)

    # Se obtiene la transformada de Fourier de la señal p_XIn para usarla en cada iteración
    ...
    for s_FreqIter in range(np.size(v_FreqTestHz)):
        # Generamos una señal sinusoidal de prueba que oscile a la frecuencia de la iteración
        # s_FreqIter (v_FreqTestHz[s_FreqIter]) y que tenga la misma longitud que la señal p_XIn.
        # En este caso usamos una exponencial compleja.
        xtest = np.exp(1j * 2.0 * np.pi * v_FreqTestHz[s_FreqIter] * v_TimeArray)

        # Creamos una ventana gaussina para limitar nuestro patrón en el tiempo
        # Definimos la desviación estándar de acuerdo al número de ciclos definidos
        # Dividimos entre 2 porque para un ventana gaussiana, una desviación estándar
        ...
        # Multiplicamos la señal patrón por la ventana gaussiana
        xtest = xtest * xtestwin

        # Para cada sinusoidal de prueba obtenemos el resultado de la convolución con la señal p_XIn
        # En este caso nos toca calcular la convolución separadamente para la parte real e imaginaria
        ...
        # Se toma únicamente la parte real para evitar corrimientos de fase
        fftxtest = abs(fftxtest)
        # Se obtine el resultado de la convolución realizando la multiplicación de las transformadas de Fourier de
